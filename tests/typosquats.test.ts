...
describe("generateTypos", () => {
  ...
  it("generates character substitution (leet-speak) variants", () => {
    const variants = generateTypos("lodash");
  ...
  it("generates leet-speak variants for express", () => {
    const variants = generateTypos("express");

    expect(variants).toContain("3xpress");
  });

  it("generates leet-speak variants for axios", () => {
    const variants = generateTypos("axios");

    expect(variants).toContain("4xios");
  });

  it("generates leet-speak variants for requests", () => {
    const variants = generateTypos("requests");

    expect(variants).toContain("reque5ts");
  });

  it("does not introduce duplicate or original-name leet variants for names with no substitutable characters", () => {
    const variants = generateTypos("xyz");

    expect(variants).not.toContain("xyz");
  });
});